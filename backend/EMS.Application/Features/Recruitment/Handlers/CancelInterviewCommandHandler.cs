...
namespace EMS.Application.Features.Recruitment.Handlers
{
    public class CancelInterviewCommandHandler : IRequestHandler<CancelInterviewCommand>
    {
        ...
        public CancelInterviewCommandHandler(IRecruitmentRepository repo, ICurrentUserService currentUser, IAuditLogger auditLogger, ILogger<CancelInterviewCommandHandler> logger)
        {
            _repo = repo;
        ...
        public async Task Handle(CancelInterviewCommand request, CancellationToken cancellationToken)
        {
            ...
            if (interview.Status != InterviewStatus.Scheduled)
                throw new InvalidOperationException($"Only a Scheduled interview can be cancelled (currently {interview.Status}).");
            ...
            _logger.LogInformation("Cancelled interview {InterviewId}", interview.Id);

            await _auditLogger.LogAsync("Interview", interview.Id, "Cancelled", ct: cancellationToken);
        }
    }
}
