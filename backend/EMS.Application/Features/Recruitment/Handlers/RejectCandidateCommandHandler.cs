...
namespace EMS.Application.Features.Recruitment.Handlers
{
    public class RejectCandidateCommandHandler : IRequestHandler<RejectCandidateCommand>
    {
        ...
        private readonly ILogger<RejectCandidateCommandHandler> _logger;

        public RejectCandidateCommandHandler(IRecruitmentRepository repo, ICurrentUserService currentUser, IAuditLogger auditLogger, ILogger<RejectCandidateCommandHandler> logger)
        {
            _repo = repo;
        ...
        public async Task Handle(RejectCandidateCommand request, CancellationToken cancellationToken)
        {
            ...
            _logger.LogInformation("Rejected candidate {CandidateId}", candidate.Id);

            await _auditLogger.LogAsync("Candidate", candidate.Id, "Rejected", ct: cancellationToken);
        }
    }
}
