...
namespace EMS.Application.Features.Payroll.Handlers
{
    public class UpdateSalaryStructureCommandHandler : IRequestHandler<UpdateSalaryStructureCommand>
    {
        ...
        public UpdateSalaryStructureCommandHandler(IPayrollRepository repo, ICurrentUserService currentUser, IAuditLogger auditLogger)
        {
            _repo = repo;
        ...
        public async Task Handle(UpdateSalaryStructureCommand request, CancellationToken cancellationToken)
        {
            ...
            if (request.Allowances != null)
                s.Allowances = request.Allowances.Select(a => new Allowance { Id = a.Id ?? Guid.NewGuid(), SalaryStructureId = s.Id, Name = a.Name, Amount = a.Amount, CreatedAtUtc = now }).ToList();

            if (request.Deductions != null)
                s.Deductions = request.Deductions.Select(d => new Deduction { Id = d.Id ?? Guid.NewGuid(), SalaryStructureId = s.Id, Name = d.Name, Amount = d.Amount, CreatedAtUtc = now }).ToList();

            await _repo.UpdateSalaryStructureAsync(s);
            await _repo.SaveChangesAsync();

            await _auditLogger.LogAsync("SalaryStructure", s.Id, "Updated", ct: cancellationToken);
        }
    }
}
