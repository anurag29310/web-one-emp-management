...
namespace EMS.Application.Features.Leave.Handlers
{
    public class RejectLeaveCommandHandler : IRequestHandler<Commands.RejectLeaveCommand>
    {
        ...
        public RejectLeaveCommandHandler(ILeaveRepository repo, IAuthRepository authRepo, IAuditLogger auditLogger, ILogger<RejectLeaveCommandHandler> logger)
        {
            _repo = repo;
            _authRepo = authRepo;
        ...
        public async Task Handle(Commands.RejectLeaveCommand request, CancellationToken cancellationToken)
        {
            ...
            await _auditLogger.LogAsync("LeaveRequest", lr.Id, "Rejected", ct: cancellationToken);
        }
    }
}
