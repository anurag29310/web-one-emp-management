...
namespace EMS.Application.Features.Clients.Handlers
{
    public class DeactivateClientCommandHandler : IRequestHandler<DeactivateClientCommand>
    {
        ...
        public DeactivateClientCommandHandler(IClientRepository repo, ICurrentUserService currentUser, IAuditLogger auditLogger, ILogger<DeactivateClientCommandHandler> logger)
        {
            _repo = repo;
        ...
        public async Task Handle(DeactivateClientCommand request, CancellationToken cancellationToken)
        {
            ...
            _logger.LogInformation("Deactivated client {ClientId}", client.Id);

            await _auditLogger.LogAsync("Client", client.Id, "Deactivated", ct: cancellationToken);
        }
    }
}
