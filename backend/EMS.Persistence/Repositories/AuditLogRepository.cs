...
namespace EMS.Persistence.Repositories
{
    public class AuditLogRepository : IAuditLogRepository
    {
        ...
        public async Task<IEnumerable<AuditLog>> GetPagedAsync(
            ...
            await BuildFilterQuery(companyId, userId, entityName, entityId, action, dateFrom, dateTo)
        ...
        public async Task<int> CountAsync(
            ...
            await BuildFilterQuery(companyId, userId, entityName, entityId, action, dateFrom, dateTo).CountAsync(ct);

        public async Task<IEnumerable<AuditLog>> GetForEntityAsync(Guid? companyId, string entityName, Guid entityId, int page, int pageSize, CancellationToken ct = default)
        {
        ...
        public async Task<int> CountForEntityAsync(Guid? companyId, string entityName, Guid entityId, CancellationToken ct = default)
        {
            var q = _db.AuditLogs.AsNoTracking()
                .Where(x => x.EntityName == entityName && x.EntityId == entityId);
            if (companyId.HasValue)
        ...
        private IQueryable<AuditLog> BuildFilterQuery(
            Guid? companyId, Guid? userId, string? entityName, Guid? entityId, string? action, DateTime? dateFrom, DateTime? dateTo)
        {
            var q = _db.AuditLogs.AsNoTracking();
