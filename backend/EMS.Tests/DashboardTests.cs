...
namespace EMS.Tests
{
    public class DashboardTests
    {
        ...
        public async Task GetSummaryAsync_ComputesEmployeeLeaveAndDepartmentCounts()
        {
            ...
            var dept = new Department { Id = Guid.NewGuid(), Name = "Engineering", CompanyId = TestCompanyId, CreatedAtUtc = DateTime.UtcNow };
            await db.Departments.AddAsync(dept);

            var activeEmp = new Employee { Id = Guid.NewGuid(), EmployeeCode = "E1", FirstName = "A", LastName = "B", IsActive = true, DepartmentId = dept.Id, CompanyId = TestCompanyId, JoinDate = DateTime.UtcNow };
            var inactiveEmp = new Employee { Id = Guid.NewGuid(), EmployeeCode = "E2", FirstName = "C", LastName = "D", IsActive = false, DepartmentId = dept.Id, CompanyId = TestCompanyId, JoinDate = DateTime.UtcNow };
            ...
            var summary = await repo.GetSummaryAsync(TestCompanyId, null, today, CancellationToken.None);
        ...
        public async Task GetSummaryAsync_FiltersByDepartment()
        {
            ...
            var deptA = new Department { Id = Guid.NewGuid(), Name = "Engineering", CompanyId = TestCompanyId, CreatedAtUtc = DateTime.UtcNow };
            var deptB = new Department { Id = Guid.NewGuid(), Name = "Sales", CompanyId = TestCompanyId, CreatedAtUtc = DateTime.UtcNow };
            await db.Departments.AddRangeAsync(deptA, deptB);

            await db.Employees.AddRangeAsync(
                new Employee { Id = Guid.NewGuid(), EmployeeCode = "E1", FirstName = "A", LastName = "B", IsActive = true, DepartmentId = deptA.Id, CompanyId = TestCompanyId, JoinDate = DateTime.UtcNow },
                new Employee { Id = Guid.NewGuid(), EmployeeCode = "E2", FirstName = "C", LastName = "D", IsActive = true, DepartmentId = deptB.Id, CompanyId = TestCompanyId, JoinDate = DateTime.UtcNow });
            await db.SaveChangesAsync();

            var attendanceRepo = new AttendanceRepository(db);
            var repo = new DashboardRepository(db, attendanceRepo);

            var summary = await repo.GetSummaryAsync(TestCompanyId, deptA.Id, DateTime.UtcNow.Date, CancellationToken.None);
        ...
        public async Task GetDailyCountsAsync_CountsAttendanceStatusesForTheGivenDate()
        {
            ...
            await db.Employees.AddRangeAsync(
                new Employee { Id = emp1, EmployeeCode = "E1", FirstName = "A", LastName = "1", CompanyId = TestCompanyId, JoinDate = DateTime.UtcNow },
                new Employee { Id = emp2, EmployeeCode = "E2", FirstName = "B", LastName = "2", CompanyId = TestCompanyId, JoinDate = DateTime.UtcNow },
                new Employee { Id = emp3, EmployeeCode = "E3", FirstName = "C", LastName = "3", CompanyId = TestCompanyId, JoinDate = DateTime.UtcNow },
                new Employee { Id = emp4, EmployeeCode = "E4", FirstName = "D", LastName = "4", CompanyId = TestCompanyId, JoinDate = DateTime.UtcNow });
            ...
            var result = await repo.GetDailyCountsAsync(TestCompanyId, date, null, CancellationToken.None);
        ...
        public async Task Handle_DefaultsToTodayWhenDateNotProvided()
        {
            ...
            var attendanceRepo = new AttendanceRepository(db);
            var repo = new DashboardRepository(db, attendanceRepo);
            var handler = new GetDashboardSummaryQueryHandler(repo, new FakeCurrentUserService(), NullLogger<GetDashboardSummaryQueryHandler>.Instance);
