...
Lightweight Employee Management System (EMS) for demo and internal use. Implements Clean Architecture with backend APIs, Angular frontend, and Docker deployment artifacts.
...
3. Backend API will be available at `http://localhost:5000` (see `docker-compose.yml`). Frontend served at `http://localhost:4200` or via nginx proxy depending on compose profile.
